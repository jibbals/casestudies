...
from utilities import fio_iris as fio # using iris for now
...
_verbose_ = True
# named latlons
_namedlatlons_=constants.latlons.items()
...
def cloud_system(model_run, hour=20, 
    ...
    hours=fio.hours_available(model_run)
    dtime=hours[hour]
    ...
    topog = fio.read_topog(model_run,extent,HSkip)
    ...
    #print(qc)
    levh  = qc.coord('altitude').points#qc.coord('level_height').points
    ...
    namedlocs_lats = []
    namedlocs_lons = []
    #print(_namedlatlons_)
    for (namedloc, (loclat, loclon)) in _namedlatlons_:
    ...
    for hi, cubetime in enumerate(cubetimes[::2]):
...
if __name__=='__main__':
    ...
    if True:
        # Images for badja
        mr="badja_run3"
        extent=[149.5843,  149.88, -36.376, -36.223]
        for hour in [15]:
            cloud_system(model_run=mr,
                         hour = hour,
                         extent = extent,
                         HSkip = 5,
                         top_height = 11500,
